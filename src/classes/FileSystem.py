...
class FileSystem:
    ...
    def changeDirectory(self, directoryName):
        if(directoryName != ".." and directoryName != 0):
            entries = self.currentDirectory.directoryEntries
            for x in entries:
            ...
            inode = self.Device.getInodeFromDisk(0)
            if (inode.i_mode == 0):
                for block in inode.i_block:
                    value = self.Device.readObject(block)
                    self.currentDirectory = value
        ...
        for x in entries:
            if(x.name == filename):
                ...
                if (inode.i_mode != 0):
                    ...
                    self.Device.writeObject(
                        self.currentDirectory, self.currentBlock)
                else:
                    return filename + " Is a directory"
